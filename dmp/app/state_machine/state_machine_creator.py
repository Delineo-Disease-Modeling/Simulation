...
from .logic.machine_naming import build_demographics_dict, build_machine_name_and_path
...
def create_state_machine(states):
    ...
    if st.session_state.current_step == 1:
        _render_step1_choose_mode(db)

    # Step 2: Disease Selection and Model Category
    elif st.session_state.current_step == 2:
        _render_step2_disease_and_category()

    # Step 3: Demographics
    elif st.session_state.current_step == 3:
        _render_step3_demographics()

    # Step 4: Edge Management and Save
    elif st.session_state.current_step == 4:
        _render_step4_edges_and_save(db)


def _render_step1_choose_mode(db):
    """Step 1: choose create-new vs edit-from-existing mode."""
...
def _render_step2_disease_and_category():
    """Step 2: pick disease, model category, variant/vaccination, and states."""
...
def _render_step3_demographics():
    """Step 3: edit the demographics rows."""
...
def _render_step4_edges_and_save(db):
    """Step 4: edge management, visualization, validation and save."""
    ...
    if render_json_edge_editor(
        ...
        st.session_state.graph_edges,
        editor_key="creator_json_editor",
        update_key="update_from_json",
        copy_key="copy_json",
        clear_key="clear_edges",
    ):
        return
    ...
    state_machine_name, model_path = build_machine_name_and_path(
        st.session_state.disease_name,
        st.session_state.model_category,
        st.session_state.variant_name,
        st.session_state.vaccination_status,
        demographics,
    )
