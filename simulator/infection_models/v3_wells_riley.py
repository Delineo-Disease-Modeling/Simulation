# FOR TESTING 
# Effectiveness of masking is 100% (i.e., no infections from masked individuals).
...
def set_droplets_passed_mask(p, droplets=1, mask_already_applied=False):
    if mask_already_applied:
        return droplets
    if p.get_masked():
        # 100% effectiveness: no droplets pass through mask
        return 0
    return droplets
        ...
        for person in p.location.population:
            ...
            if person.states.get(disease) and InfectionState.INFECTIOUS in person.states[disease]:
                if person.get_masked():
                    # 100% effectiveness: masked person contributes 0 to transmission
                    total += 0
                else:
                    total += 1
...
def calculate_frac_filtered(p, mask_already_applied=False):
    ...
    if not p.get_masked():
        return 1.0
    # 100% effectiveness: all particles are filtered out
    return 0.0
...
def CAT(p, indoor, num_time_steps, transmission_prob, infector_masked=False, susceptible_masked=False):
    ...
    # 100% mask effectiveness: complete protection
    mask_factor = 1.0
    if infector_masked or susceptible_masked:
        # If either person is masked, transmission is completely blocked
        mask_factor = 0.0
